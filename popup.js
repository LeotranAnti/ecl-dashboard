...
async function syncData() {
  ...
  try {
    ...
    // Tải song song dữ liệu các nhà máy (của Sale) và dữ liệu Telesale (của Hoạt động)
    const [results, telesaleResult] = await Promise.all([
      Promise.all(fetchPromises),
      fetchNhansuTelesaleData().catch(e => {
        console.error("Error fetching Telesale data during main sync:", e);
        return null;
      })
    ]);
    ...
    // Render cả Báo cáo Hoạt động (Nhân sự)
    ...
    // Cập nhật nhãn đồng bộ của Hoạt động
    const nhansuSyncStatus = document.getElementById("nhansu-sync-status");
    if (nhansuSyncStatus) {
      nhansuSyncStatus.textContent = `Cập nhật lúc ${timeStr}`;
    }
...
function renderNhansuDashboard() {
  ...
  if (activeDates.length > 0) {
    const aggStats = getAggregateStats(activeDates[0], activeDates[activeDates.length - 1]);
    processedStatsCount = aggStats.processedCount || 0;
  }
  setEl("ns-data-count", processedStatsCount);
